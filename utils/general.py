...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, SIoU=False, EIoU=False, WIoU=False,
             Focal=False, MPDIoU=False, alpha=1, gamma=0.5, scale=False, eps=1e-7, mpdiou_hw=None, grid=None):
    # Returns the IoU of box1 to box2. box1 is 4, box2 is nx4
    box2 = box2.T
    if MPDIoU:
        box1[:2] += grid
    ...
    if MPDIoU:
        ...
        if Focal:
            return iou - (d1 / mpdiou_hw - d2 / mpdiou_hw), torch.pow(inter / (union + eps), gamma)  # Focal_MPDIoU
        else:
