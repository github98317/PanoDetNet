    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, Focal=True)  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, Focal=True, MPDIoU=True, mpdiou_hw=pi.size(2) ** 2 + pi.size(3) ** 2, grid=torch.stack([gj, gi]))   # iou(prediction, target)
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, MPDIoU=True, mpdiou_hw=pi.size(2) ** 2 + pi.size(3) ** 2, grid=torch.stack([gj, gi]))   # iou(prediction, target)
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, alpha=3, MPDIoU=True, mpdiou_hw=pi.size(2) ** 2 + pi.size(3) ** 2, grid=torch.stack([gj, gi]))   # iou(prediction, target)
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, alpha=3)  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, WIoU=True , scale=True)  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, Focal=True, CIoU=True)  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, Focal=True, EIoU=True)  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, Focal=True, DIoU=True)  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, alpha=3, EIoU=True)  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, alpha=3, SIoU=True)  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, Focal=True, alpha=3, EIoU=True)  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, Focal=True, alpha=3)  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, alpha=3, WIoU=True, scale=True)  # iou(prediction, target)
                # print("computer loss")
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, alpha=3)  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, Focal=True)  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, Focal=True, MPDIoU=True, mpdiou_hw=pi.size(2) ** 2 + pi.size(3) ** 2 ,grid=torch.stack([gj, gi]))  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, alpha=3,  MPDIoU=True, mpdiou_hw=pi.size(2) ** 2 + pi.size(3) ** 2 ,grid=torch.stack([gj, gi]))  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, MPDIoU=True, mpdiou_hw=pi.size(2) ** 2 + pi.size(3) ** 2 ,grid=torch.stack([gj, gi]))  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, Focal=True, CIoU=True)  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, Focal=True, DIoU=True)  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, Focal=True, EIoU=True)  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, alpha=3,SIoU=True)  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, Focal=True, alpha=3, EIoU=True)  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, Focal=True, alpha=3)  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, alpha=3,EIoU=True)  # iou(prediction, target)
                # iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, alpha=3, WIoU=True, scale=True)  # iou(prediction, target)
                # print("computer loss  Ota")
                # lbox += (1.0 - iou).mean()  # iou loss
                ###修改###
                # if type(iou) is tuple:
                #     lbox += (iou[1].detach() * (1 - iou[0])).mean()
